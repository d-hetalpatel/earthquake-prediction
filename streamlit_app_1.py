...
# === LOAD TRAINED MODELS ===
drive_url = "https://drive.google.com/uc?id=19ZXoo_qSDfuabmvLrMEB7ywp7mjGywvR"
pickle_path = r"earthquake_models_and_data.pkl"

# Download the file if it doesn't exist locally
if not os.path.exists(pickle_path):
    st.info("Downloading model from Google Drive...")
    gdown.download(drive_url, pickle_path, quiet=False, fuzzy=True)
...
if os.path.exists(pickle_path):
    ...
    st.success(f"✅ Model file loaded from {pickle_path}")
else:
    st.error(f"❌ Model file not found at {pickle_path}")
    st.stop()
